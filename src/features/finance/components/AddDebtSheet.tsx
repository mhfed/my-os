...
export function AddDebtSheet({ visible, onClose }: AddDebtSheetProps) {
  ...
  const segmentAccent = isLend ? colors.green : colors.red;
  const segmentAccentDeep = isLend ? colors.greenDeep : colors.redDeep;
      ...
      <View style={styles.root}>
        ...
        >
          <View style={[styles.sheet, { paddingBottom: insets.bottom + 18 }]}>
            ...
            >
              {/* Type toggle — jelly segmented control (red/green) */}
              <View style={styles.segment}>
                {(['lend', 'borrow'] as const).map((t) => {
                  const active = type === t;
                  const accent = t === 'lend' ? colors.green : colors.red;
                  const accentDeep =
                    t === 'lend' ? colors.greenDeep : colors.redDeep;
                  return (
                    <Pressable
                      key={t}
                      onPress={() => setType(t)}
              ...
              <View style={styles.dueDateHeader}>
                ...
                >
                  <View
                    style={[styles.toggleThumb, hasDueDate && styles.toggleThumbOn]}
                  />
                </Pressable>
              </View>
              {hasDueDate && (
                      ...
                      <View style={styles.periodToggle}>
                        {(['month', 'year'] as const).map((p) => (
                          <Pressable
                            ...
                            style={[
                              styles.periodBtn,
                              interestPeriod === p && styles.periodBtnActive,
                            ]}
                          >
                            <Text
                              style={[
                                styles.periodText,
                                interestPeriod === p && styles.periodTextActive,
                              ]}
                            >
                              {p === 'month' ? 'Tháng' : 'Năm'}
                            </Text>
              ...
              <GameButton
                label={type === 'lend' ? 'Ghi nhận cho vay' : 'Ghi nhận đi vay'}
                variant={isLend ? 'green' : 'red'}
                size='md'
                icon='check-bold'
                fullWidth
                disabled={!canSubmit}
...
const styles = StyleSheet.create({
  ...
  segmentTextActive: { color: colors.white, ...textShadow.button },
  label: {
  ...
  interestChipTextActive: { color: colors.purple, fontFamily: fonts.semibold },
  ...
  periodTextActive: { color: colors.purple, fontFamily: fonts.semibold },
